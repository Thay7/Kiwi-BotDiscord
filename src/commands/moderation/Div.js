...
module.exports = class Div extends Command {
  ...
  async run({ guild, channel, author, mentions, args, msg }) {
    ...
    const embed2 = new MessageEmbed()
      ...
      .addField(`Divulgador: ${user}`, `Convidou um total de **${userInvites.reduce((p, v) => v.uses + p, 0)}** `, true)
      .addField('Convites:', `${ userInvites.size > 0 ?  userInvites.reduce((a, { code, uses }) => a + `https://discord.gg/${code} -  **${uses} membros**\n`, '') : `**${guildMember}** não possui convites no servidor.`}`, false)
